...
def introspect(token: str) -> dict[str, Any]:
    try:
        ...
        return {
            "active": active,
            "sub": payload.get("sub") if active else None,
            "typ": payload.get("typ") if active else None,
            "exp": payload.get("exp") if active else None,
            "jti": payload.get("jti") if active else None,
        }
    except Exception:
